    ...
    const renderTaskNode = (tasks, level, currentPhaseId) => {
        tasks.forEach(t => {
            ...
            tr.innerHTML = `
                ...
                ${td(1, t.isPhase ? '' : `<div style="text-align:center;"><select onchange="window.WBS_ACTION.updateTask('${effectivePid}','${t.id}','status',this.value)" onkeydown="if(event.altKey && event.key==='ArrowDown'){if(this.showPicker)try{this.showPicker()}catch(e){}}" class="status-${t.status || 'todo'}" style="width:90%; border-radius:4px;"><option value="todo" ${t.status === 'todo' ? 'selected' : ''}>To Do</option><option value="doing" ${t.status === 'doing' ? 'selected' : ''}>Doing</option><option value="done" ${t.status === 'done' ? 'selected' : ''}>Done</option></select></div>`)}
